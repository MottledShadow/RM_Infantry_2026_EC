 ...
 ******************************************************************************
 * @file    BMI088reg.h
 * @brief   BMI088 寄存器地址与位域定义（BSP 层）
 * @note    层级说明：
 *            本文件属于【BSP 层】，是纯硬件寄存器映射表。
 *            仅包含宏定义，不含任何函数或变量，可安全被任意层 include。
 *
 *          内容组织：
 *            ├── 加速度计寄存器（ACC_CHIP_ID ~ ACC_SOFTRESET）
 *            └── 陀螺仪寄存器  （GYRO_CHIP_ID ~ GYRO_SELF_TEST）
 *
 *          命名规范：
 *            BMI088_<外设>_<寄存器名>         → 寄存器地址
 *            BMI088_<外设>_<字段名>_SHFITS    → 位移量
 *            BMI088_<外设>_<字段名>           → 位掩码/配置值
 *
 *
 * @version 1.0
 * @date    2022-9-3
