 ...
 ******************************************************************************
 * @file    BMI088driver.h
 * @brief   BMI088 IMU 传感器驱动头文件（设备驱动层）
 * @note    层级说明：
 *            本文件属于【设备驱动层】，位于 BSP 层（Middleware）之上。
 *            职责：封装 BMI088 的初始化序列与数据读取，
 *                  将原始 ADC 计数值转换为物理量（m/s²、rad/s、℃）。
 *            不包含零偏补偿等算法，这些由上层 gyro_calibration 模块完成。
 *
 *          对外核心接口：
 *            BMI088_init()  → 加速度计 + 陀螺仪上电初始化与自检
 *            BMI088_read()  → 单次同步读取三轴加速度、角速度与温度
 *
 *          量程配置（修改需同步修改 BMI088driver.c 中的 SEN 变量）：
 *            加速度计：±3g（默认），可选 ±6/12/24g
 *            陀螺仪：  ±2000°/s（默认），可选 ±1000/500/250/125°/s
 *
 * @version 1.0
 * @date    2022-9-3
 * @author  DJI
